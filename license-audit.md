...
- **`icalendar-searcher==1.0.6` — AGPL-3.0-or-later — transitive (via `caldav`).**
  ...
  - Action (pick one):
    1. **Pin caldav below the icalendar-searcher floor.** caldav added the hard `icalendar-searcher` requirement in the 3.x line; pin `caldav<3` (or to the last 2.x that does not require it) if its API still satisfies `src/caldav_sync.py`, removing the AGPL dep entirely. Re-compile `requirements.txt` and re-run this audit.
