#!/bin/bash
# Build a downloadable macOS launcher app + .dmg for Telemachus.
#
#   ./build-macos-app.sh
#
# Produces:
#   dist/Telemachus.app   — double-click: starts the local server (using this
#                         repo's venv) and opens the UI in an app-style window.
#   dist/Telemachus.dmg   — drag-to-Applications disk image (the downloadable).
#
...
#!/bin/bash
# Telemachus.app — start the local server and open the UI in an app window.
...
notify() { /usr/bin/osascript -e "display notification \"$1\" with title \"Telemachus\"" >/dev/null 2>&1; }
die_gui() {
  /usr/bin/osascript -e "display dialog \"$1\" with title \"Telemachus\" buttons {\"OK\"} default button 1 with icon stop" >/dev/null 2>&1
  exit 1
}

[ -x "$UVICORN" ] || die_gui "Telemachus isn't set up yet. Open Terminal and run:
...
for i in $(seq 1 120); do
  ...
  kill -0 "$SERVER_PID" 2>/dev/null || die_gui "Telemachus failed to start. Log:
$LOG"
  sleep 1
done
...
if [ "$READY" = "1" ]; then
  ...
  notify "Telemachus is taking a while — open $URL once it finishes starting."
fi
wait "$SERVER_PID"
LAUNCHER
...
echo "Install:       open '$DIST/$APP_NAME.dmg'  (drag Telemachus to Applications)"
